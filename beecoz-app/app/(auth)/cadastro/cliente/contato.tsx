...
export default function ClienteContato(){
  const { cliente, setCliente } = useCadastro();
  const { control, handleSubmit } = useForm({ resolver: zodResolver(contatoSchema), defaultValues: { email: cliente.email || "", telefone: cliente.telefone || "" } });
  const onSubmit = (v:any)=>{ setCliente(v); router.push("/cadastro/cliente/senha"); };
