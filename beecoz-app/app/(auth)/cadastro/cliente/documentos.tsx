...
export default function ClienteDocs(){
  const { setCliente } = useCadastro();
  let rg = "", comp = "", selfie = "";
  return (
    <View style={{ flex:1, backgroundColor: theme.bg, padding:16 }}>
      <AppBarBack title="Envie seus documentos" />
      <ImagePickerField label="Foto do RG ou CPF" onChange={(u)=> rg = u }/>
      <ImagePickerField label="Comprovante de residência (últimos 3 meses)" onChange={(u)=> comp = u }/>
      <ImagePickerField label="Uma selfie nítida" onChange={(u)=> selfie = u }/>
      <Button onPress={()=>{ setCliente({ docRg: rg, docComp: comp, selfie }); router.push("/cadastro/cliente/finalizado"); }}
